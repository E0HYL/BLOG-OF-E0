...
每条指令 $$in$$ 表示为操作码 $$d$$ 维向量与 $$d$$ 维操作数向量均值的连接，即 $$2\times{d}$$ 维的 $$C\tau{(in)}=v_{P_{(in)}}^{\rightarrow}\vert\vert\frac{1}{\abs{A(in)}}\sum_t^{A(in)}v_{t_b}^{\rightarrow}$$ ；因而每个函数 $$f_s$$ 也被映射为 $$2\times{d}$$ 维的向量，记作 $$\theta_{f_s}^{\rightarrow}$$ 。
...
$$P(t_c\vert\delta(in_j,f_s))=P(v_{t'_c}^{\rightarrow}\vert\delta(in_j,f_s))=\frac{f(v_{t'_c}^{\rightarrow},\delta(in_j,f_s))}{\sum_d^Df(v_{t'_d}^{\rightarrow},\delta(in_j,f_s))} \tag{5}$$
...
$$\sum_{f_s}^{RP}\sum_{seq_i}^{S(f_s)}\sum_{in_j}^{seq_i}logP(in_j\vert{p,in_{j-1},in_{j+1}}) \tag{6}$$ 
