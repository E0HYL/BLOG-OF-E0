...
一个时间序列数据$$t$$，可表示为$$n$$个按时间顺序排列的元素$$\{x_1,\cdots ,x_n\}$$，则子序列可记作$$s=\{x_i,\cdots ,x_j\}$$。
将$$t$$划分为$$m$$个长度为$$l$$的时间段，则又可记作$$\{s_1, \cdots ,s_m\}=\{\{x_{l*k+1},\cdots ,x_{l*k+l}\}\vert 0\leq k\leq m-1\}$$。
