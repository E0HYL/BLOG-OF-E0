...
## [PV-DM 模型](https://cs.stanford.edu/~quocle/paragraph_vector.pdf)
...
训练时，如图2所示， $$\theta_{f_s}^{\rightarrow}$$ 与目标指令前后两条指令的向量平均后得 $$\delta(in_j,f_s)$$ ，则式4中的概率项可表示为 $$P(t_c\vert\delta(in_j,f_s))$$ 。
...
$$\sum_d^Df(v_{t'_d}^{\rightarrow},\delta(in_j,f_s))\approx{\sum_{i=1}^kE_{(d_t\in{P_n(t_c)})}([t_d\neq{t_c}]f(v_{t'_d}^{\rightarrow},\delta(in_j,f_s)))} \tag{7}$$

其中的 $$[·]$$ 表示恒等函数（identity function：函数参数没有任何限制，参数值永远等于函数值），参数中的表达式为真则为1，否则等于0。

<figure><img src="{{ site.url }}/images/tokenvec.PNG"  /><figcaption>图3 Asm2Vec生成的d维token向量的T-SNE聚类可视化</figcaption></figure>

## My Review

式6中f的计算要求token向量是 $$2\times{d}$$ 维的，因此多设计了该维度的映射。个人感觉token两种维度embedding的设计有点牵强。
